...
def preprocess(min_date:str = '2009-01-01', max_date:str = '2015-01-01') -> None:
    ...
    # YOUR CODE HERE
    raw_data = get_data_with_cache(query=query, filename="raw_data.csv")
    ...
    # YOUR CODE HERE
    load_data_to_bq(processed_data, table_name=f"{GCP_PROJECT_WAGON}.{BQ_DATASET}.processed_data")
...
def train(
    ...
    # YOUR CODE HERE
    X_train_processed, y_train, X_val_processed, y_val = split_data(processed_data, split_ratio)

    # Train model using `model.py`
    # YOUR CODE HERE
    model = initialize_model()
    model = compile_model(model, learning_rate)
    history = train_model(model, X_train_processed, y_train, X_val_processed, y_val, batch_size, patience)
